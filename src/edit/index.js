require(['../core/config'], function() {
    ...
    require(['jquery', 'Jcrop', 'utils', 'component'], function ($, Jcrop, utils, component) {
        $(document).ready(function () {
                    ...
                    if ($editnote != null) {
                        if ($baritem1.is(":hidden")) {
                            ...
                            if ($linktype.val() == 'link') {
                                if (!re.test($link.val())) {
                                    utils.uikit.notify({ message: "请输入正确的链接地址!" });
                                    return false;
                                }
                            }
            ...
            function targetHandle(obj, e) {
                ...
                if (isfirstTargetHandle) {
                    value = $img.children().eq(getNoteIndex()).data("id");
                            ...
                            $selectwrap.delegate("a", "click", function () {
                                ...
                                if ($this.text() == "添加") {
                                    ...
                                    if (!utils.isCompareExist(selecting, selected)) {
                                        ...
                                        utils.uikit.notify({ message: "该小时已经存在!" });
                                    }
                                } else {
                                    $cycledate.val("");
                                }
            ...
            //获得链接对象
            //链接对象有多个临时行,所以需要此方法
            function getLinkTarget(linktype) {
                var val = $linktarget.find("select:first").find("option:selected").val();
                var str = "";
                if (val == "") {
                    return str;
                } else {
                    str = " target=\"" + val + "\"";
                    if (linktype == "countdown") {
                        str += " data-id=\"" + $(".cycledate").val() + "\" datetype=\"" + $("#countdowntype").val() + "\"";
                    }
                }
                return str;
            }
